...
﻿# real_romanai_phi3_tps.py
# Copyright RomanAILabs - Daniel Harding
# Real high-TPS demo using RomanAI 4D GGUF Engine + NRL metrics
...
def run_romanai(model_path, prompt, max_tokens=200):
    try:
        # Call RomanAI native runner
        cmd = ["romanai", "run", str(model_path), "--prompt", prompt, "--max-tokens", str(max_tokens)]
